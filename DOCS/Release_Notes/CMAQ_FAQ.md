...
6. Model simulations using the STAGE deposition option and NH3 bidirectional exchange, will result in reduction of up to 10% in modeled NH3 concentrations on the CONUS domain after several months of simulation but can have larger local impacts due to the [STAGE NH3 Bidi patch](CMAQv5.3.2_bugfixes.md#14-STAGE-NH3-bidi-bugfix). This updated has reduced spuriously large emissions in areas where evasion is a sizable part of the soil NH4 budget, typically isolated grid cells in Western US with high soil pH in the CONUS domain. This should result in much more realistic estimates of NH3 evasion from these areas.  
